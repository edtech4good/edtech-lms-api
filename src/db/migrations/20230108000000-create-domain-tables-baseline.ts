...
/**
 * Sequelize 6 MySQL `attributeToSQL` ignores per-column `collate`, so mixed parent/child collations break FKs.
 * Raw DDL sets `VARCHAR(36) COLLATE` to match each referenced column.
 */
...
/**
 * Core LMS tables were never created in an earlier migration; timestamp migrations only add columns.
 * Baseline columns only — point/timestamp columns are added by `*-timestamps` migrations.
 */
module.exports = {

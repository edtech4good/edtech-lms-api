...
class LoggerInstance {
  constructor(config: LoggerConfig | null | undefined) {
    ...
    let AWSACCESSKEYID, AWSSECRETKEY, AWSREGION, LOGGERCONNECTIONSTRING;
    let loggerconfig: any = {};
    switch (LOGGERTYPE) {
      case LoggerType.ELASTICSEARCH:
        LOGGERCONNECTIONSTRING = localconfig.LOGGERCONNECTIONSTRING || (localconfig.LOGGERTYPE !== LoggerType.FILE ? error('Missing LOGGERCONNECTIONSTRING') : '');
        loggerconfig = elasticsearchlogger(LOGGERCONNECTIONSTRING, APPLICATIONNAME);
        break;
      case LoggerType.CLOUDWATCH:
        AWSACCESSKEYID = localconfig.AWSACCESSKEYID || (localconfig.LOGGERTYPE === LoggerType.CLOUDWATCH ? error('Missing AWSACCESSKEYID') : '');
        AWSSECRETKEY = localconfig.LOGGERCONNECTIONSTRING || (localconfig.LOGGERTYPE === LoggerType.CLOUDWATCH ? error('Missing AWSSECRETKEY') : '');
        AWSREGION = localconfig.LOGGERCONNECTIONSTRING || (localconfig.LOGGERTYPE === LoggerType.CLOUDWATCH ? error('Missing AWSREGION') : '');

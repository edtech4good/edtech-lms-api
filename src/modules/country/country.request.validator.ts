...
const createcountry: RequestValidator = {
  body: joi.object().keys({
    countryname: joi
      ...
      .label("Country Name"),
    expectedusage: joi.number().min(0).message("Invalid expected usage"),
  }),
};
...
const showcountry: RequestValidator = {
  params: joi.object().keys({
    countryid: joi.string().required().uuid().label("Country ID"),
  }),
};

const showallcountry: RequestValidator = {
  body: joi.object().keys(<SchemaMap<IPaging>>{
    ...
    filter: joi
      .array()
      .items({
        key: joi.string().valid("countryname").required(),

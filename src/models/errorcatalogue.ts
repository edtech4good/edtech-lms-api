import { HttpStatus } from '@nestjs/common';
import { ErrorCode } from './enums/errorcode.enum';

export interface ErrorCatalogueEntry {
  status: number;
  errormessage: string;
  hint?: string;
}

/**
 * Default status/message/hint per ErrorCode, straight out of
 * docs/api-errors.md. A throw site may override `errormessage` and/or `hint`
 * with something more specific (e.g. "That file is larger than 10 MB."), but
 * must still follow the "never leak" rules in that doc — see ApiError.
 */
export const ErrorCatalogue: Record<ErrorCode, ErrorCatalogueEntry> = {
  [ErrorCode.LOGIN_FAILED]: {
    status: HttpStatus.BAD_REQUEST,
    errormessage: 'The username or password is incorrect.',
    hint: "Check both and try again. If you've forgotten your password, ask your school.",
  },
  [ErrorCode.SIGN_IN_REQUIRED]: {
    status: HttpStatus.UNAUTHORIZED,
    errormessage: 'Please sign in to continue.',
    hint: 'Your session may have expired.',
  },
  [ErrorCode.NOT_ALLOWED]: {
    status: HttpStatus.FORBIDDEN,
    errormessage: "You don't have permission to do that.",
    hint: 'If you think you should, ask your school administrator.',
  },
  [ErrorCode.NOT_FOUND]: {
    status: HttpStatus.NOT_FOUND,
    errormessage: "We couldn't find that.",
    hint: 'It may have been removed or moved.',
  },
  [ErrorCode.INVALID_INPUT]: {
    status: HttpStatus.BAD_REQUEST,
    errormessage: "Some of the information isn't valid.",
    hint: 'Check the highlighted fields.',
  },
  [ErrorCode.ALREADY_EXISTS]: {
    status: HttpStatus.CONFLICT,
    errormessage: 'That already exists.',
    hint: 'Use a different name, or edit the existing one.',
  },
  [ErrorCode.FILE_REJECTED]: {
    status: HttpStatus.BAD_REQUEST,
    errormessage: "That file can't be used.",
  },
  [ErrorCode.TOO_MANY_ATTEMPTS]: {
    status: HttpStatus.TOO_MANY_REQUESTS,
    errormessage: 'Too many attempts.',
    hint: 'Wait a minute, then try again.',
  },
  [ErrorCode.SERVICE_UNAVAILABLE]: {
    status: HttpStatus.SERVICE_UNAVAILABLE,
    errormessage: 'The service is temporarily unavailable.',
    hint: 'Try again in a few minutes. Your work is kept and retried.',
  },
  [ErrorCode.INTERNAL]: {
    status: HttpStatus.INTERNAL_SERVER_ERROR,
    errormessage: 'Something went wrong on our side.',
    hint: 'Try again. If it keeps happening, contact support with the reference.',
  },
};

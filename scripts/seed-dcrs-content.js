...
/**
 * DCRS demo content for the central LMS: a real client deck — Mekong
 * Inclusive Ventures' (MIV) "Disabilities Capital Readiness" programme,
 * Cohort II, "Module 1: Business Vision & Goals" — loaded as a curriculum
 ...
 * first and the other still finds it. edtech-lms-rpi-api's seed-dcrs-content.js
 * uses the identical ID map, so the two databases end up looking as they would
...
const ID = {
  ...
  curriculumcountry: "c0000000-0000-4000-8000-000000000004", // central only
...
async function main() {
  ...
  try {
    ...
    // unlike seed-demo-content.js's running index across all lessons).
    const PRACTICE_MAP = [
    ...
    const [[counts]] = await conn.query(`
      SELECT (SELECT COUNT(*) FROM students WHERE schoolname = 'Mekong Inclusive Ventures') AS students,
             (SELECT COUNT(*) FROM lessons WHERE levelid = ?)                                AS lessons,
             (SELECT COUNT(*) FROM questions WHERE questionidentifier LIKE 'DCRS-%')         AS questions`,
      [ID.level]);

    console.log("DCRS content seeded.");
    console.log(`  students ${counts.students} | lessons ${counts.lessons} | questions ${counts.questions}`);
    console.log(`  facilitator login: miv.facilitator / ${DEMO_PASSWORD}`);
    console.log(`  student login:     miv.demo / ${DEMO_PASSWORD}`);
